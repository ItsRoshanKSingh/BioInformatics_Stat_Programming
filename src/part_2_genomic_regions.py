...
def extract():
    with open("output\\files\\free_energy.txt", "r") as energy_file:
        free_energy_values = [float(value) for value in energy_file.read().split(",")]

    promoter_regions = []
    non_promoter_regions = []

    with open("resources\\ecoli_k12_tss.txt", "r") as tss_file:
    ...
    with open("output/files/promoter_regions.csv", "w") as file:
        print(
    ...
    with open("output/files/non_promoter_regions.csv", "w") as file:
        print(
